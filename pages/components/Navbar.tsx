...
export default function Navbar() {
  ...
  return (
    <>
      <div className="flex items-center justify-between p-5 border-b border-b-black mb-5">
        <Link href={"/"} className="resotho text-lg">
          FITPAL
        </Link>
        <TbMenu className="md:hidden" onClick={showMenu} />
        <div className="hidden resotho md:flex gap-x-4">
      ...
      <div className="hidden flex flex-col gap-y-3 text-lg resotho h-screen menu px-5">
        {updatedMenu.map((item, index) => {
          if (item.name === "Signout")
            return (
              <span onClick={() => signOut(() => router.push("/"))} key={index} className="text-left">
